...
function Blogs() {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged((user)=>{
            if(user){
                if(user.uid==="ZL101xjjrch3ww9MwAPhWne1qMO2"){
                    setShowBlog(true)
                }
                else{
                    setShowBlog(false)
        ...
        axios.get("https://blog-project-backend-ilto.onrender.com/api/blogs").then((res) => {
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`https://blog-project-backend-ilto.onrender.com/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("https://blog-project-backend-ilto.onrender.com/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
    ...
    return (
        <div className="blog-section py-14">
            ...
            {/* Blog creation form */}
           {showBlog? <div className="blog-creation-form mb-8" style={{ width: "80%", margin: "auto" }}>
            ...
            <div className="blogs-container grid grid-cols-1 md:grid-cols-2 gap-6 container mx-auto px-4">
                ...
                {blogs.map((blog) => (
                    <div key={blog._id} className="blog-post mb-8 p-6 bg-slate-200 shadow-lg rounded-lg">
